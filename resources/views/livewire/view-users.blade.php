    ...
    <div class="w-full h-full">
        <main class="mx-auto w-[60%] font-inter gap-12 my-24">
            <header class="flex justify-between px-4 py-4 background-secondary-color mb-12 rounded-sm shadow-sm">
                <div>
                    <select
                        class="text-white cursor-pointer background-primary-color hover:bg-blue-800 focus:ring-1 focus:outline-none focus:ring-blue-50 font-medium rounded-lg text-sm px-2 py-2.5 text-center inline-flex items-center dark:bg-blue-500 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                        ...
                        @foreach ($users as $user)
                            <li class="flex items-center justify-between py-2 px-4 border-b-1 border-gray-500">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <a
                                        href="{{ route('admin_update_profile', ['id' => Crypt::encrypt($user->usr_id)]) }}">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            stroke-width="1.5" stroke="black"
                                            class="size-8 hover:stroke-gray-600 transition-all duration-100 ease">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                    ...
                                    <form method="POST" action="{{ route('delete_user') }}">
                                        ...
                                        <input name="id" type="hidden"
                                            value="{{ Crypt::encrypt($user->usr_id) }}">
